...
description: "nvm 설치와 셀 설정, 버전 설치·전환, .nvmrc, 자주 쓰는 명령어와 pyenv와의 차이를 정리한다."
---

> **nvm (Node Version Manager)** 는 여러 Node.js 버전을 설치·전환하는 도구다. pyenv 와 달리 미리 컴파일된 **바이너리를 내려받고**, `nvm` 자체는 실행 파일이 아니라 셀에 로드되는 **셀 함수**로 동작한다.
...
설치 스크립트가 `~/.zshrc` 에 아래를 자동으로 추가한다. 없으면 직접 넣고 셀을 다시 로드한다.
...
```bash
nvm use 22             # 현재 셀에서 사용할 버전 전환
nvm use --lts
nvm alias default 22   # 새 셀이 기본으로 쓸 버전 지정
nvm current            # 현재 활성 버전
```

> `nvm use` 는 **현재 셀에만** 적용된다. 새 터미널을 열면 `nvm alias default` 로 지정한 버전으로 돌아간다.
...
| `nvm ls` | 설치된 버전 목록 |
| `nvm ls-remote` | 설치 가능한 원격 버전 목록 |
| `nvm use <ver>` | 현재 셀의 버전 전환 |
| `nvm alias default <ver>` | 새 셀의 기본 버전 지정 |
...
- **셀 함수** — `nvm` 은 `nvm.sh` 를 source 해서 얻는 셀 함수다. 그래서 셀마다 로드가 필요하고, 스크립트·`Makefile` 같은 비대화형 셀에서는 안 잡힐 수 있다.
- **전역 npm 패키지 분리** — 버전마다 global 패키지가 따로 관리된다. 전환 후 다시 깔거나 `--reinstall-packages-from` 으로 옮겨야 한다.
- **자동 전환 없음** — `.nvmrc` 자동 적용은 셀 훅을 직접 추가해야 한다(pyenv 의 자동 전환과 가장 큰 차이).
...
| 동작 원리 | 셀 함수 (source) | shim (PATH 가로채기) |
